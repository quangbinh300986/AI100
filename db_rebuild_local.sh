...
# 连入本地 AI100 数据库，重建发布，包含所有 17 张核心业务表
...
if [ $? -eq 0 ]; then
    echo "✔ 本地发布重建成功: battle_pub (包含 17 张表)"
else
...
docker exec -t supabase-db pg_dump -U postgres -d AI100 --schema=public > /tmp/local_AI100_rebuild.sql

if [ $? -eq 0 ]; then
    echo "✔ 本地数据库备份成功: /tmp/local_AI100_rebuild.sql"
else
    echo "❌ 本地数据库备份失败！"
    exit 1
fi
...
echo "提示：请在下方按提示输入云端服务器 (106.55.22.207) 的 root 密码"

# 传送到云服务器的 /tmp 目录
scp /tmp/local_AI100_rebuild.sql root@106.55.22.207:/tmp/
